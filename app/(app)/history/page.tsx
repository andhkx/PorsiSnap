...
export default function HistoryPage(){
  ...
  return <div className="mx-auto max-w-5xl p-4 pb-28 space-y-4">
    <div className="flex items-center justify-between gap-3">
      <div className="neo-badge bg-[var(--neo-lavender)]">Riwayat</div>
      <Link href="/foto" className="neo-btn !py-2 !px-4 bg-[var(--primary)] text-white !rounded-full text-xs">+ Snap</Link>
    </div>
    <div className="neo-card bg-white p-6">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        ...
        <div className="neo-card-soft !p-3 bg-white"><div className="text-2xl font-black leading-none">{target}</div><div className="mt-1 text-[10px] font-black uppercase tracking-widest">Target</div></div>
      ...
      : <div className="mt-4 space-y-2">
        {items.map(it=>(
          <div key={it.id} className="flex gap-3 neo-card-soft !p-2 items-center bg-white">
            {it.foto_url? <img src={it.foto_url} alt={it.nama_makanan} className="h-16 w-16 shrink-0 rounded-xl object-cover border-2 border-[#0f172a]"/> : <div className="grid h-16 w-16 shrink-0 place-items-center rounded-xl bg-[var(--muted)] border-2 border-[#0f172a] text-xl" aria-hidden>🍱</div>}
            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              ...
              <button type="button" onClick={()=>del(it.id)} className="neo-badge bg-[var(--neo-coral)] !text-[10px]">Hapus</button>
            </div>
          </div>
        ))}
      </div>}
